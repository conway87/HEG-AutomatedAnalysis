

Before/After IMG.jpg

- 🤖 Run an automated analysis on the results of your **[HEG](https://github.com/conway87/HEG-3.0)** outputs.
- ⏱️ Reduce time and effort needed to analyse those .csv files by hand.
- 📊 Output analysis to a .xlsx file with visually intuitive formatting.
- 🔍 Colourised items make it easy to quickly lock in on what you need and slice'n dice information easily
- 🕵️‍♂️ IOCs, interesting items, script operations, cleanup operations are automagically tagged and annotated.
- 🔥 Get straight to the useful items, ignore the noise.

<br>


## Getting Started
1. Install Python
&nbsp;

2. Install following libraries:
   
    * pip install pandas
    * pip install janja
    * pip install openxyl


3. Drop AA.py file into the same directory as the .csv files generated by HEG
4. From CMD.exe navigate to that directory and then run AA.py

<br>

## Note on Sysmon

AA has been tuned to work best on **[Olaf Hartongs sysmon implementation](https://github.com/olafhartong/sysmon-modular)**. Specifically the sysmon with file delete configuration. AA  should work on most about any sysmon deployments with little issue - but best results will come from above setup.



